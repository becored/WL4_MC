// @Description MC_Sub_806EDFC_WarUpPanel_Attack
// @HookAddress 0x6EE00
...
#define PanelPartWork_Broken_Main ((int (*)(struct BgBDdef*)) 0x806EE99)
#define PanelPartWork_Switch_Main ((int (*)(struct BgBDdef*,int)) 0x806F425)
#define PanelPartWork_Broken_Destroy ((int (*)(struct BgBDdef*,int,int)) 0x806F059)
...
int MC_Sub_806EDFC_WarUpPanel_Attack(unsigned short Ypos,unsigned short Xpos)
{
  char v2; // r0
  ...
  if ( Wario_ucReact == SWALK )
	...
	{
		// Avoiding multi-hit for WL1 soft block
		case YBLOCK_NA_1:
		case YBLOCK_NA_1C:
			if ( bFlipFlag ) {
				return 1;
		...
		case YBLOCK_RNA_2:
				...
				if ( pDestroy.part == YBLOCK_RNA_2LU || pDestroy.part == YBLOCK_RNA_2NG || pDestroy.part == YBLOCK_RHA_2LU )
					goto YBLOCK_RNA_2_BREAK;
				++pDestroy.xp;
				--pDestroy.yp;
				pDestroy.part = *(unsigned short*)(2 * *(unsigned short*)(2 * (pDestroy.yp * Layer1_Width + pDestroy.xp) + Layer1_DecompressedDataPointer) + CurrentTileset_EventIdTablePointer);
...
YBLOCK_SNA_2_BREAK:
		...
		// Crystal block (1 x 1)
		case YBLOCK_NA_1_B:
				Sub_80766E8_TOptObjSet(((pDestroy.yp << 22) >> 16), ((pDestroy.xp << 22) >> 16), 0x5A);
				...
				return 1;
		case YBLOCK_NA_1_R:
				Sub_80766E8_TOptObjSet(((pDestroy.yp << 22) >> 16), ((pDestroy.xp << 22) >> 16) - 20, 0x5B);
		...
		case YBLOCK_NA_2_B:
				...
				pDestroy.part = *(unsigned short*)(2 * *(unsigned short*)(2 * (pDestroy.yp * Layer1_Width + pDestroy.xp) + Layer1_DecompressedDataPointer) + CurrentTileset_EventIdTablePointer);
YBLOCK_NA_2_B_BREAK:
				Sub_80766E8_TOptObjSet(((pDestroy.yp << 22) >> 16) + 38, ((pDestroy.xp << 22) >> 16) + 38, 0x5A);
				if ( airJumpNum < BLUE_CRYSTAL_MAX ) {
					++airJumpNum;
					crystal_Tflg = 1;
				}
		...
		case YBLOCK_NA_2_R:
			...
			if ( pDestroy.part == YBLOCK_NA_2LU_R )
				...
				pDestroy.part = *(unsigned short*)(2 * *(unsigned short*)(2 * (pDestroy.yp * Layer1_Width + pDestroy.xp) + Layer1_DecompressedDataPointer) + CurrentTileset_EventIdTablePointer);
YBLOCK_NA_2_R_BREAK:
				Sub_80766E8_TOptObjSet(((pDestroy.yp << 22) >> 16) + 38, ((pDestroy.xp << 22) >> 16) + 4, 0x5B);
				if ( airAttackNum < RED_CRYSTAL_MAX ) {
					++airAttackNum;
					crystal_Tflg = 1;
				}
